...
def parse_http_response(text_response):
    ...
    # разделяем строку статуса и заголовки/контент
    ...
    for index, line in enumerate(lines):
        ...
        if line == '':
            ...
            # запомнить индекс с которого начинается контент и завершить цикл.
            empty_index = index
            break
        print(line)
        # в случае, если мы не дошли до контента, значит мы всё еще находимся
        # на блоке с заголовками - разбиваем заголовки по символу `:` и
        # сохраняем в словарь название и значение заголовка.
        k, _, v = line.partition(':')
        headers.setdefault(k.strip(), v.strip())
    # индекс пустой строки - empty_index.
    # для взятия контента, надо использовать все нижележащие строки после
...
# создаем TCP-сокет (IP) и подключаемся к домену
